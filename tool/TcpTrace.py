...
BPF_PERF_OUTPUT(send_ipv4_events);

struct send_ipv6_data_t {
    u64 time;
    char name[32];
    u32 pid;
    u32 ppid;
...
BPF_PERF_OUTPUT(send_ipv6_events);
...
int kprobe__tcp_sendmsg(struct pt_regs *ctx, struct sock *sk, struct msghdr *msg, size_t size)
{
    ...
    u32 sa_family = sk->__sk_common.skc_family;
    if (sa_family == 2) {
        struct send_ipv4_data_t data4 = {.pid = pid, .ppid = ppid};
        ...
        send_ipv4_events.perf_submit(ctx, &data4, sizeof(data4));
    } else if(sa_family == 10){
        struct send_ipv6_data_t data6 = {.pid = pid, .ppid = ppid};
        ...
        send_ipv6_events.perf_submit(ctx, &data6, sizeof(data6));
    }
...
def print_send_ipv4_event(cpu, data, size):
    event = b["send_ipv4_events"].event(data)

    skip = False
    if args.name and not re.search(bytes(args.name), event.task):
        skip = True
    if not skip:
        print("send data[%s]"% strftime("%H:%M:%S"))
        print("%-7s %-7s %-15s %-32.32s %-6s" % ("PID", "PPID", "COMM", "DADDR", "DPORT"))
...
def print_send_ipv6_event(cpu, data, size):
    event = b["send_ipv6_events"].event(data)
...
b["send_ipv4_events"].open_perf_buffer(print_send_ipv4_event)
b["send_ipv6_events"].open_perf_buffer(print_send_ipv6_event)
while True:
    try:
        b.perf_buffer_poll()
